...
@click.option('-d', '--db', help = 'default: /titan/softwares/PlasT/plasmids_202303.repr.fas', default='/titan/softwares/PlasT/plasmids_202303.repr.fas')
@click.option('-g', '--group', help='default: /titan/softwares/PlasT/plasmids_202303.repr.clu', default='/titan/softwares/PlasT/plasmids_202303.repr.clu')
@click.option('--blastn', help='executable for blastn', default='/usr/bin/blastn')
    ...
    while data.shape[0] :
        ...
        for cont, regions in contigs.items() :
            ...
            for c, s in cont_seqs.items() :
                cont_info[cont][c] = [np.sum(s), np.sum(s)/np.sum((s>0)), np.sum((s>0))/contLen[cont]*100.]
        clusters = {}
